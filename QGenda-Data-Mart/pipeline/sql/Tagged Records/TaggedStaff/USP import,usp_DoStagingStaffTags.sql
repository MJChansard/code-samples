/*	FILE HEADER
 *		File Name:	USP import,usp_DoStagingStaffTags.sql
 ...
 *		the following tasks:
 *			- Ensures that the source data only contains one type of Tag record (Staff or Task)
 *			- Identify new records that need to be inserted in the reporting data (ANESCore)
...
ALTER PROCEDURE import.usp_DoStagingStaffTags
AS
BEGIN
	SET NOCOUNT ON;
	
	DECLARE @NewRecords TABLE (EntityKey UNIQUEIDENTIFIER);
	...
	SET @mixedRecords = 
		CASE
			WHEN (SELECT DISTINCT EntityType FROM import.TagsAPI) <> 'Staff'
			THEN 1
			ELSE 0
		END
	...
	ELSE 
		BEGIN
			INSERT INTO @NewRecords 
				...
				FROM stage.qdm_TaggedStaff;

			INSERT INTO import.qdm_TaggedStaff (StaffKey)
				SELECT EntityKey FROM @NewRecords
				UNION
				SELECT StaffKey FROM stage.qdm_TaggedStaff;

			-- Only thing not considered here is records that exist in ANESCore but do not exist in the API data
			-- This should not occur as even Staff Members who have departed can still be find in the API data

			UPDATE import.qdm_TaggedStaff
			SET 
			-- InvalidRecordFlag
				InvalidRecordFlag = 
					CASE
						WHEN StaffKey IN
							(
								'11DEEF29-D633-4744-BEE4-9E2E11DDA682'		--	RRNA Bulletin
								, '4C714E84-1C20-4C4A-B78A-BD2DAAFB854E'	--	Deactivated Deactivated
								, '8FB8AC78-0953-4B23-8202-F22B332898D0'	--	Physicians Physicians
								, 'EF2B722B-EAB1-4DE9-9FB8-E01F52ADEEAE'	--	PST PA PST PA
								, '20DFD0E3-0ED6-4D72-8281-B0D4F6C44F10'	--	Staff QG
								, 'DF41E1F2-15E1-4299-A283-D1C47101715A'	--	RESIDENTS RESIDENTS
								, '8DFCB0FE-E5FD-47DB-9CAD-D217257D9FF0'	--	Block MD MD
								, 'F12CABBD-D70D-455F-A043-66F1889FD1C8'	--	NP/PA NP/PA
								, 'E7D4D478-8774-4C26-80D6-B89A65942CCC'	--	UH CRNA UH CRNA
								, 'A220ACAB-66A8-41A9-AD5C-CD27A5FBE2EF'	--	UH RRNA UH RRNA
								, '640BD4EE-0EAD-4991-890B-A89E8AAB0ACE'	--	Bulletin CRNA
								, '8A75E010-FDCD-430E-91D4-B9F45D885A0F'	--  Fellows Fellows
							)
						THEN 1
						ELSE 0
					END
			...
			-- Tag Category: Capacity Tab
				, Capacity_Cardiothoracic =
					CASE
						WHEN i.Tags LIKE '%"Key":393623,"Name":"Cardiothoracic Capacity"%' THEN 1
						ELSE 0
					END
			...
			-- Tag Category: CUH
				, CUH_CUH10hr = 
					CASE
						WHEN i.Tags LIKE '%"Key":531515,"Name":"CUH 10hr"%' THEN 1
						ELSE 0
					END
				, CUH_CUH13hr = 
					CASE
						WHEN i.Tags LIKE '%"Key":531516,"Name":"CUH 13hr"%' THEN 1
						ELSE 0
					END
			-- Tag Category: Division
				...
				, Division_CUHGeneralPrimary =
					CASE
						WHEN i.Tags LIKE '%"Key":308250,"Name":"CUH General Primary"%' THEN 1
						ELSE 0
					END
				, Division_CUHOB = 
					CASE
				...
				, Division_PHHSOBCUHCore = 
					CASE
						WHEN i.Tags LIKE '%"Key":495476,"Name":"PHHS OB CUH Core"%' THEN 1
						ELSE 0
					END
				, Division_PHHSOBHybrid =	-- NO RECORDS WITH THIS TAG FOUND 05-06-2022
					CASE
						WHEN i.Tags LIKE '%"Key":,"Name":""%' THEN 1
						ELSE 0
					END
			...
			-- Tag Category: MD - Simulation
				, MDSimulation_SIM = 
					CASE
						WHEN i.Tags LIKE '%"Key":334265,"Name":SIM"%' THEN 1
						ELSE 0
					END
			-- Tag Category: N/D
				, ND_Day = 
			...
			-- Tag Category: QGenda Admin Tags
				, QGendaAdminTags_Header =
					CASE
						WHEN i.Tags LIKE '%"Key":320341,"Name":"Header"%' THEN 1
						ELSE 0
					END
				, QGendaAdminTags_LBL =
					CASE
						WHEN i.Tags LIKE '%"Key":320342,"Name":"LBL"%' THEN 1
						ELSE 0
					END
			-- Tag Category: Staff Primary Location
				, StaffPrimaryLocation_CUH = 
					CASE
						WHEN i.Tags LIKE '%"Key":254992,"Name":"CUH"%' THEN 1
						ELSE 0
					END
				, StaffPrimaryLocation_Zale =
					CASE
						WHEN i.Tags LIKE '%"Key":254979,"Name":"Zale"%' THEN 1
						ELSE 0
					END
				, TTCMMockPunch_MDs = 
					CASE
						WHEN i.Tags LIKE '%"Key":315432,"Name":"MDs"%' THEN 1
						ELSE 0
					END
			FROM import.qdm_TaggedStaff AS s
				INNER JOIN import.TagsAPI AS i
					ON s.StaffKey = i.EntityKey AND i.EntityType = 'Staff';
			PRINT 'Parsing of tag data for staff members completed.';
			...
			SELECT
				...
				, i.InvalidRecordFlag				AS iInvalidRecord
				, s.InvalidRecordFlag				AS sInvalidRecord
				, cInvalidRecord					= IIF(i.InvalidRecordFlag = s.InvalidRecordFlag, 'Match', 'Conflict')
				...
				-- Tag Category: CUH
				, i.CUH_CUH10hr						AS iCUH_CUH10hr
				, s.CUH_CUH10hr						AS sCUH_CUH10hr
				, cCUH_CUH10hr						= IIF(i.CUH_CUH10hr = s.CUH_CUH10hr, 'Match', 'Conflict')
				
				, i.CUH_CUH13hr						AS iCUH_CUH13hr
				, s.CUH_CUH13hr						AS sCUH_CUH13hr
				, cCUH_CUH13hr						= IIF(i.CUH_CUH13hr = s.CUH_CUH13hr, 'Match', 'Conflict')
				...
				, i.Division_PHHSOBCUHCore			AS iDivision_PHHSOBCUHCore
				, s.Division_PHHSOBCUHCore			AS sDivision_PHHSOBCUHCore
				, cDivision_PHHSOBCUHCore			= IIF(i.Division_PHHSOBCUHCore = s.Division_PHHSOBCUHCore, 'Match', 'Conflict')
				...
				-- Tag Category: MD - Simulation
				, i.MDSimulation_SIM				AS iMDSimulation_SIM
				, s.MDSimulation_SIM				AS sMDSimulation_SIM
				, cMDSimulation_SIM					= IIF(i.MDSimulation_SIM = s.MDSimulation_SIM, 'Match', 'Conflict')
				...
				-- Tag Category: Staff Primary Location
				, i.StaffPrimaryLocation_CUH		AS iStaffPrimaryLocation_CUH
				, s.StaffPrimaryLocation_CUH		AS sStaffPrimaryLocation_CUH
				, cStaffPrimaryLocation_CUH			= IIF(i.StaffPrimaryLocation_CUH = s.StaffPrimaryLocation_CUH, 'Match', 'Conflict')

				, i.StaffPrimaryLocation_Zale		AS iStaffPrimaryLocation_Zale
				, s.StaffPrimaryLocation_Zale		AS sStaffPrimaryLocation_Zale
				, cStaffPrimaryLocation_Zale		= IIF(i.StaffPrimaryLocation_Zale = s.StaffPrimaryLocation_Zale, 'Match', 'Conflict')
				-- Tag Category: TTCM Mock Punch
				, i.TTCMMockPunch_MDs				AS iTTCMMockPunch_MDs
				, s.TTCMMockPunch_MDs				AS sTTCMMockPunch_MDs
				, cTTCMMockPunch_MDs				= IIF(i.TTCMMockPunch_MDs = s.TTCMMockPunch_MDs, 'Match', 'Conflict')
			INTO #Comparison
			FROM import.qdm_TaggedStaff AS i
				INNER JOIN stage.qdm_TaggedStaff AS s
			...
			-- Correct changes to InvalidRecordFlag
			INSERT INTO @RecordsToUpdate
				SELECT StaffKey
				FROM #Comparison
				WHERE cInvalidRecord = 'Conflict';

			IF (SELECT COUNT(*) FROM @RecordsToUpdate) > 0
				BEGIN
					UPDATE stage.qdm_TaggedStaff
					SET InvalidRecordFlag = i.InvalidRecordFlag
						, ETLCommand = 'Update'
					FROM stage.qdm_TaggedStaff AS s
						INNER JOIN import.qdm_TaggedStaff AS i
			...
			-- TAG CATEGORY: CUH
			-- Correct changes to CUH 10hr
			INSERT INTO @RecordsToUpdate
				SELECT StaffKey
				FROM #Comparison
				WHERE cCUH_CUH10hr = 'Conflict';

			IF (SELECT COUNT(*) FROM @RecordsToUpdate) > 0
				BEGIN
					UPDATE stage.qdm_TaggedStaff
					SET CUH_CUH10hr = i.CUH_CUH10hr
						, ETLCommand = 'Update'
					FROM stage.qdm_TaggedStaff AS s
						INNER JOIN import.qdm_TaggedStaff AS i
							ON s.StaffKey = i.StaffKey
			...
			-- Correct changes to CUH 13hr
			INSERT INTO @RecordsToUpdate
				SELECT StaffKey
				FROM #Comparison
				WHERE cCUH_CUH13hr = 'Conflict';

			IF (SELECT COUNT(*) FROM @RecordsToUpdate) > 0
				BEGIN
					UPDATE stage.qdm_TaggedStaff
					SET CUH_CUH13hr = i.CUH_CUH13hr
						, ETLCommand = 'Update'
					FROM stage.qdm_TaggedStaff AS s
						INNER JOIN import.qdm_TaggedStaff AS i
							ON s.StaffKey = i.StaffKey
		...
		-- TAG CATEGORY: Integrations
			-- Correct changes to Integrations_Kronos
			INSERT INTO @RecordsToUpdate
				SELECT StaffKey
				FROM #Comparison
				WHERE cIntegrations_Kronos = 'Conflict';

			IF (SELECT COUNT(*) FROM @RecordsToUpdate) > 0
				BEGIN
					UPDATE stage.qdm_TaggedStaff
					SET Integrations_Kronos = i.Integrations_Kronos
						, ETLCommand = 'Update'
					FROM stage.qdm_TaggedStaff AS s
						INNER JOIN import.qdm_TaggedStaff AS i
							ON s.StaffKey = i.StaffKey
			...
			-- TAG CATEGORY: MDSimulation
			-- Correct changes to MDSimulation_SIM
			INSERT INTO @RecordsToUpdate
				SELECT StaffKey
				FROM #Comparison
				WHERE cMDSimulation_SIM = 'Conflict';

			IF (SELECT COUNT(*) FROM @RecordsToUpdate) > 0
				BEGIN
					UPDATE stage.qdm_TaggedStaff
					SET MDSimulation_SIM = i.MDSimulation_SIM
						, ETLCommand = 'Update'
					FROM stage.qdm_TaggedStaff AS s
						INNER JOIN import.qdm_TaggedStaff AS i
							ON s.StaffKey = i.StaffKey
		...
		-- TAG CATEGORY: QGenda Admin Tags
			...
			-- TAG CATEGORY: Staff Primary Location
			-- Correct changes to StaffPrimaryLocation_CUH
			INSERT INTO @RecordsToUpdate
				SELECT StaffKey
				FROM #Comparison
				WHERE cStaffPrimaryLocation_CUH = 'Conflict';

			IF (SELECT COUNT(*) FROM @RecordsToUpdate) > 0
				BEGIN
					UPDATE stage.qdm_TaggedStaff
					SET StaffPrimaryLocation_CUH = i.StaffPrimaryLocation_CUH
						, ETLCommand = 'Update'
					FROM stage.qdm_TaggedStaff AS s
						INNER JOIN import.qdm_TaggedStaff AS i
							ON s.StaffKey = i.StaffKey
			...
			-- Correct changes to StaffPrimaryLocation_Zale
			INSERT INTO @RecordsToUpdate
				SELECT StaffKey
				FROM #Comparison
				WHERE cStaffPrimaryLocation_Zale = 'Conflict';

			IF (SELECT COUNT(*) FROM @RecordsToUpdate) > 0
				BEGIN
					UPDATE stage.qdm_TaggedStaff
					SET StaffPrimaryLocation_Zale = i.StaffPrimaryLocation_Zale
						, ETLCommand = 'Update'
					FROM stage.qdm_TaggedStaff AS s
						INNER JOIN import.qdm_TaggedStaff AS i
							ON s.StaffKey = i.StaffKey
			...
			-- Correct changes to TTCMMockPunch_MDs
			INSERT INTO @RecordsToUpdate
				SELECT StaffKey
				FROM #Comparison
				WHERE cTTCMMockPunch_MDs = 'Conflict';

			IF (SELECT COUNT(*) FROM @RecordsToUpdate) > 0
				BEGIN
					UPDATE stage.qdm_TaggedStaff
					SET TTCMMockPunch_MDs = i.TTCMMockPunch_MDs
						, ETLCommand = 'Update'
					FROM stage.qdm_TaggedStaff AS s
						INNER JOIN import.qdm_TaggedStaff AS i
							ON s.StaffKey = i.StaffKey
		...
		-- #NEW RECORDS
			INSERT INTO stage.qdm_TaggedStaff
				SELECT 
					StaffKey
					, InvalidRecordFlag
					...
					, TTCMMockPunch_MDs
					, 'New' AS PushToProductionFlag
				FROM import.qdm_TaggedStaff AS d
					INNER JOIN @NewRecords AS fltr
						ON d.StaffKey = fltr.EntityKey;
